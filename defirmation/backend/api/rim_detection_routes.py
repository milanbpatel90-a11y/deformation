...
def get_rim_engine() -> RimDetectionEngine:
    ...
    if _rim_engine is None:
        # Try to load trained YOLOv8 model if it exists
        yolo_path = Path("runs/detect/eyewear_seg/weights/best.pt")
        _rim_engine = RimDetectionEngine(
            yolo_model_path=str(yolo_path) if yolo_path.exists() else None
        )
