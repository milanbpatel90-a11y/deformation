...
class MeasurementExtractor:
  ...
  For MVP, uses pixel ratios calibrated against typical product photo framing.
    ...
    def extract_from_images(
        ...
        lens_height     = self._clamp(lens_height,    20.0,  56.0, "lens_height")
    ...
    def _clamp(value: float, lo: float, hi: float, name: str = "") -> float:
        """Hard-clamp a measurement to a physically plausible range."""
        if value < lo or value > hi:
            import warnings
            warnings.warn(
                f"Measurement '{name}' = {value:.1f}mm is out of range "
                f"[{lo}, {hi}] — clamped.",
                stacklevel=4,
            )
    ...
    def _measure_rim_thickness(self, top: np.ndarray) -> float:
        """
        Estimate rim thickness from the top-view image.

        Uses its own pixel scale: temple arm typically spans 120-155mm and
        occupies most of the image width.
        """
        ...
        # Scale: the whole top-view image width ≈ temple_length ≈ 140mm
        mm_per_px_top = 140.0 / max(w_img, 1)
        thickness_mm = ch * mm_per_px_top
        return round(max(0.8, min(6.0, thickness_mm)), 2)

    def _measure_temple(self, side: np.ndarray) -> tuple[float, float]:
        """
        Measure temple length from the side-view image.

        Uses its own pixel scale derived from the side image dimensions.
        A side-view product photo typically shows the full temple arm
        (120-155mm) spanning ~60-80% of the image width.
        """
        ...
        # Find the horizontally longest contour bounding box — that's the temple arm
        longest = max(contours, key=lambda c: cv2.boundingRect(c)[2])
        _, _, cw, ch = cv2.boundingRect(longest)

        # Scale: assume the temple arm spans ~70% of the side-image width → 140mm
        mm_per_px_side = 140.0 / max(w_img * 0.70, 1)
        temple_length = cw * mm_per_px_side

        # Curve angle from direction of the longest open contour
        all_pts = max(contours, key=lambda c: cv2.arcLength(c, False))
        if len(all_pts) >= 3:
