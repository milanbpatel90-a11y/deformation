...
class Auto3DPipeline:
    ...
    def process_image(
        ...
        shape, detected_material, nose_pads = self.shape_classifier.classify(
            image, masks.get("front")
        )
        ...
        if hasattr(self.segmenter, '_model') and self.segmenter._model is not None:
            ...
            measurements, lens_contour = self._extract_from_opencv_masks(
                image, masks, shape, material, color, nose_pads
            )
        
        # Save measurements
        measurements_path = output_dir / f"{image_path.stem}_measurements.json"
        with open(measurements_path, 'w') as f:
            json.dump(measurements.model_dump(), f, indent=2)
        ...
        return {
            ...
            "measurements_data": measurements.model_dump(),
            "shape": shape.value,
